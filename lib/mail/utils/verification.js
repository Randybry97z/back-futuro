...
const sendVerificationEmail = async function (usuario, verificationOption, admin) {
  ...
  _bcryptjs.default.hash(uniqueString, saltRound).then(hashedUniqueString => {
    ...
    _models.default.Token.create({
      ...
      _mailer.transporter.sendMail(verificationOption == 1 ? mailOptions : verificationOption == 0 ? mailDeleteOptions : verificationOption == 2 ? mailForgotPswOptions : verificationOption, (err, info) => {
        console.log(info);
        console.log(err);
      });
    }).catch(error => {
